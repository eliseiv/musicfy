...
### Два платёжных контура

Монеты пополняются двумя независимыми путями:

1. **Прямой StoreKit** — клиент сам отдаёт JWS-чек в `POST /v1/billing/purchases/verify`
   (и `POST /v1/billing/restore` при переустановке).
2. **Adapty** — server-to-server вебхук `POST /v1/billing/adapty/webhook`, который вызывает
   сам Adapty. Клиенту довозить чек не нужно; достаточно вызвать `Adapty.identify(<deviceId>)`
   тем же `deviceId`, который передаётся в `POST /v1/auth/guest` — по нему сервер и находит
   пользователя, которому начислять монеты.

⚠️ Одна сборка приложения должна использовать **ровно один** контур. Если чек пройдёт обоими
путями, монеты начислятся дважды (дедуп-ключи контуров не пересекаются).
