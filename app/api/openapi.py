...
Лимиты раздельны по категориям: **song**, **cover**, **video**.

- Сначала тратится подписочный лимит (entitlements), затем покупные паки.
- Текущий баланс: `GET /v1/billing/balance`.
- При исчерпании лимита создание задачи вернёт **402 `INSUFFICIENT_CREDITS`** —
  это сигнал показать paywall.
- `lyrics` и `voice clone` кредиты не тратят.
...
```json
{
  "error": {
    ...
    "details": { "category": "song", "required": 1 }
  },
