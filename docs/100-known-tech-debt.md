...
| [TD-014](#td-014) | Дедуп покупок между StoreKit- и Adapty-контурами не пересекается — одна покупка, прошедшая обоими путями, начислит монеты дважды | high | open |
...
## TD-014 — Дедуп не пересекается между платёжными контурами {#td-014}

**Контекст:** ADR-019 §Риск. Ключи идемпотентности начисления лежат в разных namespace:
StoreKit-путь пишет `purchase:{dedup_key}` (ADR-013), Adapty-путь — `adapty-txn:{transaction_id}`.
`uq_credit_ledger_idempotency_key` не поймает пересечение, потому что строки ключей разные.
Итог: одна и та же покупка, доехавшая обоими путями (клиент отдал чек в `purchases/verify` И
Adapty прислал вебхук), начислит монеты ДВАЖДЫ.

**Почему не сведено в один namespace:** StoreKit-ключ содержит `environment`
(`Production` / `Sandbox` / `Xcode`) — он и разделяет области дедупа (ADR-013 D1/D2). Adapty
`environment` транзакции не передаёт, поэтому вычислить эквивалентный ключ из его payload
нечем. Схождение потребовало бы дедупа по «голому» `transaction_id`, что вернуло бы дефект,
ради которого ADR-013 и вводил environment-scoping.

**Митигация сейчас — контрактная:** одна сборка приложения использует ровно один платёжный
путь. Сборка, отдающая чеки в `purchases/verify`, не должна быть подключена к Adapty-вебхуку.
Это соглашение, а не инвариант кода — нарушение обнаружится только по жалобе или аудиту ledger.

**Закрытие:** либо (а) ретировать StoreKit-путь после полного перехода на Adapty (тогда риск
исчезает вместе со вторым контуром), либо (б) ввести кросс-контурный дедуп по
`original_transaction_id` с явной таблицей соответствия, если Adapty начнёт отдавать
`environment`.
...
## TD-011 — `APPLE_STOREKIT_ENVIRONMENT` — возможный dead-config после ADR-013 {#td-011}

- **Контекст:** [ADR-013](./adr/ADR-013-storekit-dedup-environment-scoping.md) перевёл определение
  окружения транзакции на **верифицированный payload** (claim `environment`), а не на настройку
  сервера. `APPLE_STOREKIT_ENVIRONMENT` (`config.py:98`, `Literal["Sandbox","Production"]`,
  дефолт `Sandbox`) определена, но grep по коду не находит её чтения — вероятно вестигиальна.
- **Долг:** переменная либо (а) реально нужна App Store **Server API** клиенту для выбора base URL
  Apple (Sandbox vs Production) при server-to-server запросах истории/нотификаций — тогда её надо
  явно подключить и задокументировать; либо (б) остаток до-ADR-013 логики — тогда удалить (код +
  `.env.example`), чтобы не вводить в заблуждение (её значение сейчас ни на что не влияет).
- **Серьёзность — low:** функционального эффекта нет (переменная не читается); риск — только
  когнитивный (ложное впечатление, что окружение задаётся конфигом). Реальный дискриминатор —
  claim из payload (ADR-013 §D1), fail-fast и trust-anchor от этого долга не зависят.
- **Триггер закрытия:** backend подтверждает роль в server-to-server клиенте
  ([Q-BILL-3](./99-open-questions.md#q-bill-3)) → закрыть как «используется» + документировать;
  иначе удалить переменную.

---

## TD-010 — Пин StoreKit Test root-сертификата — ручная ops-операция {#td-010}

- **Контекст:** [ADR-013](./adr/ADR-013-storekit-dedup-environment-scoping.md) §D3. Чтобы
  Xcode-транзакции (local testing) принимались криптографически, а не через глобальный bypass
  подписи, корневой сертификат StoreKit Test конкретной машины разработчика пинится в
  `APPLE_STOREKIT_TEST_ROOT_CERTS` (список PEM, дефолт — пустой).
- **Долг:** сертификат генерируется Xcode'ом локально — при смене машины, переустановке Xcode или
  подключении второго iOS-разработчика список нужно обновлять руками (env → редеплой). Автоматики
  (self-service enrollment / ротация) нет.
- **Обход для Testing-режима ([ADR-014](./adr/ADR-014-storekit-cn-trust-xcode-flag.md)):** флаг
  `APPLE_STOREKIT_TRUST_XCODE_TEST_CERTS=true` включает CN-trust — доверие любому self-signed
  EC-сертификату с `CN="StoreKit Testing in Xcode"` без пина DER, что **снимает** эту ops-нагрузку
  на время Testing-режима (не нужно пинить каждую машину). Ценой сознательного per-user риска
  (майнинг коинов себе); перед публичным релизом флаг выключается (DEPLOYMENT.md §8). Пин по DER
  остаётся как более безопасная точечная альтернатива, если риск CN-trust нежелателен.
- **Серьёзность — low:** операция редкая, аудируемая и по определению осознанная; штатный путь
  тестирования — staging-окружение (`APP_ENV=dev`), где пин не нужен. Прод-инвариант
  (`APPLE_STOREKIT_VERIFY_SIGNATURE=true`, fail-fast) от этого долга не страдает.
- **Триггер закрытия:** появление второго-третьего iOS-разработчика или частая смена машин →
  вынести доверенные тест-корни в БД (таблица + admin-эндпоинт) вместо env.
