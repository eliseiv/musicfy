...
- **Гостевой вход идемпотентен по устройству** ([ADR-020](./adr/ADR-020-guest-sign-in-device-idempotency.md)):
  `POST /v1/auth/guest` с уже известным `deviceId` возвращает новую сессию ТОМУ ЖЕ гостю
  (прежняя сессия не отзывается), а не создаёт нового пользователя. Клиент зовёт endpoint на
  каждом холодном старте, поэтому без этого второй запуск приложения падал в 500 на
  `uq_auth_identities_provider_subject`, теряя кошелёк и историю устройства. Устройство,
  принадлежащее промоутнутому аккаунту (`is_guest=false`), по `deviceId` не выдаётся —
  иначе знание идентификатора заменяло бы Sign in with Apple.
...
Решение — [ADR-007](./adr/ADR-007-video-three-modes.md); параметры генерации (style/audio/размер)
уточнены [ADR-016](./adr/ADR-016-video-generation-params-fix.md).
...
**Параметры seedance-генерации (ADR-016, сверено с реальной схемой seedance-2.0 t2v):**

- **style → prompt.** У seedance нет поля `style`; стиль подмешивается в `prompt` детерминированным
  server-side суффиксом (`_apply_style`, маппинг `VideoStyle → фраза` в `pipelines/video.py`):
  realistic → `photorealistic, natural lighting, ...`; cartoon → `cartoon style, vibrant flat colors, ...`;
  anime → `anime style, cel-shaded, ...`; cinematic → `cinematic, film grain, dramatic lighting, ...`.
  Применяется **всегда** (и при явном prompt / surprise_me) для visual_clip (t2v+i2v) и lyrics-фона.
  Пользовательская часть промпта модерируется как и сейчас; суффикс — фиксированная константа, не
  модерируется.
- **generate_audio=false** для всех наших t2v/i2v-сабмитов — трек пользователя муксим сами, своя
  аудиодорожка seedance не нужна (устраняет двойное аудио + вес). Инвариант пайплайна, не env.
- **resolution=720p** (env `FAL_VIDEO_RESOLUTION`) — явное поле сабмита (баланс качество/размер/цена).
  `duration` опционален (дефолт seedance `auto`; кап через `FAL_VIDEO_MAX_DURATION`, т.к. visual_clip
  зацикливает клип под длину трека).
- **Детерминированный размер (< 80 МБ, типично 40–60) — в существующем re-encode** mux/lyrics-render
  (без отдельной стадии): `libx264 -profile:v high -pix_fmt yuv420p -crf 26 -maxrate 2500k
  -bufsize 5000k`, `aac -b:a 128k`, `-movflags +faststart` (обязательно для галереи iOS/Telegram),
  без апскейла выше 720p. Кап `maxrate` гарантирует размер независимо от битрейта source (устраняет
  0.5 ГБ). Avatar-выход пока не нормализуется — [TD-012](./100-known-tech-debt.md#td-012).
- **aspect_ratio:** t2v — работает (значения `VideoAspect` = enum seedance). i2v — шлём best-effort,
  но соотношение от картинки не гарантируется; avatar — не применяется (не шлём). Кроп/пад под
  запрошенный aspect отложен — [TD-013](./100-known-tech-debt.md#td-013).

Контракт fal-client (`base.py` Protocol + `client.py` + `stub.py`): `submit_text_to_video` /
`submit_image_to_video` / `submit_lyrics_background` получают `resolution` / `generate_audio` /
`duration` (только поддерживаемые seedance поля, `None` не шлётся); `submit_lipsync_video` /
`submit_avatar_image_video` — без изменений.
...
|---|---|---|---|---|
| `100_tokens_9.99` | coin_pack | 100 Tokens | `{"coins":100}` | — |
| `250_tokens_19.99` | coin_pack | 250 Tokens | `{"coins":250}` | — |
...
инвариант — иначе продления старых подписок и restore ломаются); только `GET /v1/billing/products`
(`list_active`) отдаёт клиенту 7 активных продуктов. `generation_prices` (цены генераций) и отсутствие
колонки `price` у `products` — без изменений.

### Trust anchor и окружение транзакции ([ADR-013](./adr/ADR-013-storekit-dedup-environment-scoping.md) §D3)

Подпись JWS проверяется **всегда**; корень цепочки `x5c` определяет доверие и окружение:

- корень = закреплённый **Apple Root CA - G3** → payload подлинный, claim `environment`
  (`Production` / `Sandbox`) доверенный;
...
Ключ дедупа вычисляется **из окружения транзакции** (криптографически удостоверенного, см. выше),
а не из конфига сервера. Единый источник истины — `BillingService._dedup_key(user_id, tx)`:

| environment | Ключ | Область |
|---|---|---|
| `Production` | `Production:{transaction_id}` | глобально (replay-защита: чужой чек не применить) |
| `Sandbox` | `Sandbox:{transaction_id}` | глобально, отдельный namespace от Production |
| `Xcode` | `Xcode:{user_id}:{transaction_id}:{purchase_date_ms}` | на пользователя + момент покупки |

`transaction_id` в Xcode local testing — счётчик (`0`, `1`, …), обнуляемый при *Delete All
Transactions*; `purchase_date_ms` даёт энтропию, поэтому повторная покупка после сброса начисляется,
а повтор того же payload — дедуплицируется (идемпотентность по полному payload сохранена).

**Оба слоя используют один ключ** (их расхождение и было причиной бага «первая покупка молча не
начисляется»):

- `purchases`: `UNIQUE (dedup_key)` = `uq_purchases_dedup_key`, вставка через
  `on_conflict_do_nothing`. Старый `uq_purchases_transaction_id` снят → неуникальный
  `ix_purchases_transaction_id`. Колонки `environment`, `purchase_date` — аудит + материал ключа;
  тестовые начисления на проде отделимы от боевых (`environment != 'Production'`).
- `credit_ledger`: `idempotency_key = f"purchase:{dedup_key}"`.
- Монеты начисляются **только при `newly is True`** (строка `purchases` реально вставлена); оба
  insert'а — в одной транзакции, частичного состояния не бывает.

Подписки: у каждого продления новый `transaction_id` → новый ключ → грант за период;
`_update_subscription_state` — идемпотентный upsert.

### Контракт ответа применения транзакции ([ADR-013](./adr/ADR-013-storekit-dedup-environment-scoping.md) §D1/§D2)

`POST /v1/billing/purchases/verify` → `ApplyResultResponse`; `POST /v1/billing/restore` →
`list[ApplyResultResponse]` (по элементу на каждую переданную транзакцию). Схема (camelCase в JSON):

```
ApplyResultResponse = { status: "ok" | "rejected" | "ignored", deduplicated: bool, reason: string | null }
```

`reason` **не теряется**: для `rejected`/`ignored` он несёт машиночитаемую причину. Матрица:

| Ситуация | Ответ |
|---|---|
| Монеты начислены сейчас (строка `purchases` реально вставлена) | `{ status:"ok", deduplicated:false, reason:null }` |
| Свой повтор (restore / повторный verify того же чека) | `{ status:"ok", deduplicated:true, reason:null }` |
| Replay чужого чека (`dedup_key` занят **другим** `user_id`) | `{ status:"rejected", deduplicated:false, reason:"transaction_already_claimed" }` |
| Payload без эффекта | `{ status:"ignored", reason:"unknown_product" \| "incomplete_transaction" }` |

`status="rejected"` и `status="ignored"` — **не успех**: монеты не начислены, клиент обязан их
различать (не трактовать любой `2xx` как «покупка засчитана»). Источник статусов — `BillingService`
(`verify_and_apply_transaction` → `_apply`); replay-ветка логируется `WARNING` (`environment`,
`transaction`) без утечки чужого `user_id` в ответ.

Миграция `0016_purchase_dedup_key` (`down_revision = "0015_soft_delete"`) добавляет колонки,
переносит констрейнт и **обязательно бэкфиллит** `credit_ledger.idempotency_key`
(`purchase:{tx}` → `purchase:Production:{tx}`) — без этого шага restore после деплоя начислил бы
монеты повторно.
