...
| [Q-BILL-4](#q-bill-4) | Берёт ли iOS `product_id` из `GET /v1/billing/products`? Если да — витрина на doravixo отдаст не тот регистр, нужен per-instance каталог. | Биллинг | open |

---

## Q-BILL-4 — Витрина каталога и регистр `product_id` на мульти-инстансе {#q-bill-4}

[ADR-021](./adr/ADR-021-product-id-case-insensitive-resolution.md) сделал **резолв** продукта
регистронезависимым, поэтому начисление монет работает при любом регистре, приходящем от
StoreKit или Adapty. Осталась **витрина**: `GET /v1/billing/products` отдаёт
`external_product_id` в том виде, в каком он лежит в БД (строчном), а в App Store Connect у
`doravixo.shop` продукты названы с заглавной — `100_Tokens_9.99`.

**Вопрос владельцу/iOS:** клиент хардкодит `product_id` для запроса к StoreKit или берёт их из
ответа этого endpoint'а?

- **Хардкодит** → вопрос закрывается без изменений, каталог остаётся общим.
- **Берёт из API** → на `doravixo` StoreKit не найдёт продукты по строчным id, и понадобится
  **per-instance каталог**: свой набор `external_product_id` на инстанс (seed/env вместо общей
  миграции). Это отдельный ADR и заметная переделка сида каталога, поэтому не делалось
  превентивно.

Начисления вопросом не затронуты ни при каком ответе.
